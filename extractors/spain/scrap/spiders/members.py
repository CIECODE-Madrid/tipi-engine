...
class MemberSpider(CrawlSpider):
    ...
    rules.append(
            Rule(LinkExtractor(
                allow=['diputadosBajaLegActual&paginaActual=\d'], unique=True), follow=True))
    ...
    def birthdate_extract(self, text, name):
        day = int(re.findall('\s+([0-9]{1,2})\s', text)[0])
        month = self.month_parser(re.findall(
    ...
    def parse_member(self, response):
        ...
        curriculum = Selector(response).xpath('//div[@class="texto_dip"]/ul/li/div[@class="dip_rojo"]')
        ...
        avatar = Selector(response).xpath('//div[@id="datos_diputado"]/p[@class="logo_grupo"]/img[@name="foto"]/@src').extract()
        ...
        birthdate = Selector(response).xpath('((//div[@class="texto_dip"])[2]/ul/li)[1]').extract()[0]
        next_index = 2
        # There are some deputies without birth dates
        if 'Nacid' not in birthdate:
            birthdate = ''
            next_index = 1
        legislatures = Selector(response).xpath('((//div[@class="texto_dip"])[2]/ul/li)[{}]'.format(next_index)).extract()
        bio = Selector(response).xpath('((//div[@class="texto_dip"])[2]/ul/li)[{}]'.format(next_index+1)).extract()
        ...
        if names:
            ...
            if birthdate:
                item['birthdate'] = self.birthdate_extract(self.text_cleaner(birthdate), item['name'])
            if start_date:
            ...
            if extra_data:
                web_data = Selector(response).xpath('//div[@class="webperso_dip"]/div[@class="webperso_dip_parte"]/a/@href')
                if web_data:
